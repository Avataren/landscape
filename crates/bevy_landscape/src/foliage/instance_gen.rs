...
use super::{FoliageConfig, FoliageInstance, FoliageLodTier};
use super::generation::{blend_density_masks, write_mask_tile};
use super::tiles::FoliageTileWriter;
...
pub fn bake_and_write_foliage_instances(
    ...
    let (slope_mask, altitude_mask, water_mask) = {
        ...
        let slope = compute_slope_density(
            height_data, tile_size, world_scale, height_scale, config.slope_threshold,
        );
        let altitude = compute_altitude_density(
            height_data, height_scale, config.altitude_min, config.altitude_max,
        );
        let water = compute_water_proximity_density(
            height_data, height_scale, water_level, config.water_proximity_falloff,
        );
    ...
    let procedural_mask_path = super::procedural_mask_path(foliage_root, mip_level, tx, ty);
    ...
    for lod_tier in FoliageLodTier::all() {
        let instances = generate_tile_instances(
            tile_size, mip_level, &painted_splatmap, &procedural_mask,
            world_scale, config, *lod_tier, tx, ty,
        );
...
mod tests {
    ...
    fn test_generate_tile_instances_empty() {
        ...
        let painted = vec![0u8; 256 * 256];
        let procedural = vec![0u8; 256 * 256];
        let instances = generate_tile_instances(
            256, 0, &painted, &procedural, 1.0, &config, FoliageLodTier::Lod0, 0, 0,
        );
    ...
    fn test_generate_tile_instances_lod_multiplier() {
        ...
        let painted = vec![255u8; 256 * 256];
        let procedural = vec![255u8; 256 * 256];
        let lod0 = generate_tile_instances(256, 0, &painted, &procedural, 1.0, &config, FoliageLodTier::Lod0, 0, 0);
        let lod1 = generate_tile_instances(256, 0, &painted, &procedural, 1.0, &config, FoliageLodTier::Lod1, 0, 0);
        let ratio = lod1.len() as f32 / lod0.len() as f32;

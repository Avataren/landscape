...
fn fill_layer(fill: [u8; 4]) -> Vec<u8> {
    solid_layer(PBR_TEX_RESOLUTION, PBR_TEX_RESOLUTION, &fill)
}

fn make_pbr_array(layers: Vec<Vec<u8>>) -> Image {
    build_texture_2d_array(
        &layers,
        TextureArrayConfig {
            width: PBR_TEX_RESOLUTION,
            height: PBR_TEX_RESOLUTION,
            layers: MAX_SHADER_MATERIAL_SLOTS as u32,
            format: TextureFormat::Rgba8Unorm,
            usage: RenderAssetUsages::RENDER_WORLD,
            sampler: TextureArraySampler::repeat_linear_mip(Some(8)),
            view_dimension: Some(TextureViewDimension::D2Array),
        },
        MipFilter::Rgba8Triangle,
    )
}

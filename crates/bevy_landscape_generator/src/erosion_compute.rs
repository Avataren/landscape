...
impl FromWorld for ErosionPipeline {
    fn from_world(world: &mut World) -> Self {
        let uniform_entries =
            BindGroupLayoutEntries::sequential(ShaderStages::COMPUTE, (uniform_buffer::<ErosionUniform>(false),));
        let uniform_layout =
        ...
        Self { uniform_layout, textures_layout, pipelines }
    }
}
...
fn prepare_delta_height_buffer(
    ...
    let Some(eb) = erosion_buffers else { return; };
    let resolution = eb.resolution;
    if existing.map(|b| b.resolution == resolution).unwrap_or(false) {
        return;
    }
...
fn prepare_erosion_textures_bind_group(
    ...
    if existing.map(|bg| bg.resolution == eb.resolution).unwrap_or(false) {
        return;
    }

    let Some(ha)   = gpu_images.get(&eb.height_a)   else { return; };
    let Some(hb)   = gpu_images.get(&eb.height_b)   else { return; };
    let Some(wt)   = gpu_images.get(&eb.water)       else { return; };
    let Some(sed)  = gpu_images.get(&eb.sediment)    else { return; };
    let Some(flux) = gpu_images.get(&eb.flux)        else { return; };
    let Some(vel)  = gpu_images.get(&eb.velocity)    else { return; };
    let Some(hard) = gpu_images.get(&eb.hardness)    else { return; };
    let Some(raw)  = gpu_images.get(&ni.raw_heights) else { return; };

    let entries = [
        BindGroupEntry { binding: 0, resource: BindingResource::TextureView(&ha.texture_view) },
        BindGroupEntry { binding: 1, resource: BindingResource::TextureView(&hb.texture_view) },
        BindGroupEntry { binding: 2, resource: BindingResource::TextureView(&wt.texture_view) },
        BindGroupEntry { binding: 3, resource: BindingResource::TextureView(&sed.texture_view) },
        BindGroupEntry { binding: 4, resource: BindingResource::TextureView(&flux.texture_view) },
        BindGroupEntry { binding: 5, resource: BindingResource::TextureView(&vel.texture_view) },
        BindGroupEntry { binding: 6, resource: BindingResource::TextureView(&hard.texture_view) },
        BindGroupEntry {
            binding: 7,
        ...
        BindGroupEntry { binding: 8, resource: BindingResource::TextureView(&raw.texture_view) },
    ];
...
enum ErosionNodeState { Loading, Ready }

pub(crate) struct ErosionNode { state: ErosionNodeState }

impl Default for ErosionNode {
    fn default() -> Self { Self { state: ErosionNodeState::Loading } }
}
...
impl Node for ErosionNode {
    fn update(&mut self, world: &mut World) {
        ...
        let ok = |id: CachedComputePipelineId| matches!(
            cache.get_compute_pipeline_state(id),
            CachedPipelineState::Ok(_)
        );
        let pl = &pipeline.pipelines;
        if matches!(self.state, ErosionNodeState::Loading)
            && ok(pl.copy_in) && ok(pl.init_hardness) && ok(pl.clear_buffers)
            && ok(pl.copy_out)
            && ok(pl.water_add) && ok(pl.flux_update) && ok(pl.water_velocity)
            && ok(pl.erode_deposit) && ok(pl.sediment_transport)
            && ok(pl.copy_b_to_sediment) && ok(pl.evaporate)
            && ok(pl.clear_delta) && ok(pl.thermal_compute) && ok(pl.thermal_apply)
            && ok(pl.particle_erode) && ok(pl.particle_apply)
        {
    ...
    fn run(
        ...
        if matches!(self.state, ErosionNodeState::Loading) { return Ok(()); }

        let Some(ep) = world.get_resource::<ErosionParams>() else { return Ok(()); };
        if !ep.enabled { return Ok(()); }

        let Some(ctrl) = world.get_resource::<ErosionControlState>() else { return Ok(()); };
        let Some(ubg) = world.get_resource::<ErosionUniformBindGroup>() else { return Ok(()); };
        let Some(tbg) = world.get_resource::<ErosionTexturesBindGroup>() else { return Ok(()); };
        ...
        macro_rules! dispatch {
            ($label:expr, $pl:expr, $x:expr, $y:expr, $z:expr) => {{
                let mut pass = render_context.command_encoder().begin_compute_pass(
                    &ComputePassDescriptor { label: Some($label), ..default() },
                );
        ...
        if done == 0 {
            ...
            dispatch!("erosion_init_hardness", p_init_hardness, wg_xy.0, wg_xy.1, 1);
            dispatch!("erosion_clear_buffers", p_clear_buffers, wg_xy.0, wg_xy.1, 1);
        }
        ...
        for _tick in 0..ticks {
            ...
            dispatch!("hydro_water_velocity",     p_water_velocity,      wg_xy.0, wg_xy.1, 1);
            dispatch!("hydro_erode_deposit",      p_erode_deposit,       wg_xy.0, wg_xy.1, 1);
            dispatch!("hydro_sediment_transport", p_sediment_transport,  wg_xy.0, wg_xy.1, 1);
            dispatch!("copy_b_to_sediment",       p_copy_b_to_sediment,  wg_xy.0, wg_xy.1, 1);

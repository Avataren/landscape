...
//
// Each LOD level has its own R8Unorm layer covering its ring.
// The LOD index is derived from the patch scale baked into the model matrix.
...
fn height_at(lod: u32, xz: vec2<f32>) -> f32 {
    ...
    // lvl.z  = 1 / ring_span  →  maps world coords into [0,1] UV
...
fn vertex(v: Vertex) -> TerrainVOut {
    ...
    // --- Height from this LOD's clipmap layer. ---
    let h   = height_at(lod_level, world_xz);
    let pos = vec3<f32>(world_xz.x, h, world_xz.y);

    // --- Normal via central differences. ---
    // eps ≈ one clipmap texel in world units for this LOD (clip_levels[L].w).
    let eps = terrain.clip_levels[lod_level].w;
    let h_r = height_at(lod_level, world_xz + vec2<f32>(eps, 0.0));
    let h_u = height_at(lod_level, world_xz + vec2<f32>(0.0, eps));
